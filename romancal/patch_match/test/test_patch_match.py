...
def mk_im_corners(ra, dec, pa, size):
    ...
    center = sgv.lonlat_to_vector(0., 0.)
    ...
    trect = [rotate(*(rotate(*(vec + yaxis + (-dec,))) + zaxis + (ra,)))
        for vec in rect]
...
def mk_gwcs(ra=cra, dec=cdec, pa=cpa, bounding_box=None, pixel_shape=None):
    ...
    """
    transform = (
        (amm.Shift(-2048) & amm.Shift(-2048))
        | (amm.Scale(0.11/3600.) & amm.Scale((0.11/3600.))
        | amm.Rotation2D(pa)
        | amm.Pix2Sky_TAN()
        | amm.RotateNative2Celestial(cra, cdec, 180.)))
...
@pytest.mark.parametrize("pars, expected",
    [((cra, cdec + e, cpa, csize), (925051, 925151)),
     ...
     ((cra, cdec, cpa, 0.5),
          (924750, 924751, 924849, 924850, 924851, 924852, 924948, 924949, 924950,
           924951, 924952, 924953, 925047, 925048, 925049, 925050, 925051, 925052,
           925053, 925054, 925147, 925148, 925149, 925150, 925151, 925152, 925153,
           925154, 925248, 925249, 925250, 925251, 925252, 925253, 925349, 925350,
           925351, 925352, 925450, 925451))])
