...
def create_psf_matched_image(
    ...
    if model.err is not None:
        # Preserve units from original error array
        err_unit = getattr(model.err, "unit", None)
        # Strip units before variance calculation to avoid unit complications
        if err_unit is not None:
            err_value = model.err.value
        else:
            err_value = model.err

        variance = err_value**2
        ...
        # Restore units if they existed
        if err_unit is not None:
            matched_err = matched_err * err_unit
    else:
        matched_err = None
...
def compute_psf_correction_factors(
    ...
    3. Computing correction factors C = flux_matched / flux_original
    ...
    for flux_col in flux_columns:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            correction = flux_matched / flux_original
            # Set correction to 1.0 where original flux is zero or
            # invalid
            correction = np.where(
                (flux_original == 0) | ~np.isfinite(correction), 1.0, correction
            )
