...
def calc_transforms_coarse(t_pars: TransformParameters):
    """
    COARSE calculation.

    This implements the overall coarse-mode transfomations as defined by the
    STScI Innerspace document "Quaternion Transformations for Coarse Pointing".
    ...
    is given by the following formula. Each term is a 3x3 matrix::
        ...
        where
            ...
            M_z_to_x    : Transform between ICS frame to Idl frame
            M_v_to_siaf : Transform from the V-frame to Detector frame (as defined by the SIAF)
            M_eci_to_v  : Transform from the Observatory attitude (quaternion) to V-frame

    """
    logger.info(
        "Calculating transforms using COARSE Tracking..."
    )
    t_pars.method = Methods.COARSE

    # Determine the M_eci_to_v matrix. Since this is a full train, the matrix
...
def calc_m_eci2v(t_pars: TransformParameters):
    """
    Calculate the M_eci2v matrix as presented in the STScI Innerspace document "Quaternion Transforms for Coarse Pointing WCS"

    Notes
    -----
    M_eci2v represents the transformation from observatory orientation,
    as defined by the engineering quaternion (ECI), to
    the V-frame of the optical telescope assembly. The transformation is

        M_eci_to_v =
            transposition(M_v_to_fcs) *
            transposition(M_fcs_to_gs) *
            M_x_to_z *
            M_eci_to_gs

        where

            M_v_to_fcs : FCS reference point in the V_frame
