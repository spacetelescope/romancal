# Contributing to the Roman Telescope Calibration Pipeline

`romancal` is an open source package written in Python.
The source code is available in the [`romancal` Github repository](https://github.com/spacetelescope/romancal/).
...
[open an issue](https://github.com/spacetelescope/romancal/issues) or contact the [Roman Telescope Help Desk](https://romanhelp.stsci.edu).
...
1. [Fork this repository](https://github.com/spacetelescope/romancal/fork).
2. Clone your fork to your local machine:
    ```shell
    git clone https://github.com/YOUR_USERNAME/romancal
    cd romancal/
    ```

3. Add the `upstream` repository, as a remote, to your local clone:
    ```shell
    git remote add upstream https://github.com/spacetelescope/romancal
    ```
...
5. [Install `romancal` to your development environment.](#creating-a-development-environment)
...
8. [Open a new Pull Request](https://github.com/spacetelescope/romancal/pulls) requesting that your changes be merged into the `main` branch of this repository.
...
As `romancal` is constantly evolving, you will often encounter the situation where you've made changes to your branch, but in that time there are new commits on `upstream/main` from other developers.
...
When developing `romancal` (or any other Python package), you should install the package locally to a development environment.
...
```shell
mamba create -n romancal_dev_env python=3.13
mamba activate romancal_dev_env
pip install -e .
hx .
```

Breaking down what these lines do:
1. Create a new empty environment called `romancal_dev_env`:
   ```shell
   mamba create -n romancal_dev_env python=3.13
   ```
2. "Activate" the environment (change shell variables in the current session to point to the isolated Python installation):
   ...
   mamba activate romancal_dev_env
   ```
3. Install the local package (`romancal`) to your environment in "editable mode", so that any code changes will be instantly reflected in the installed package (useful for testing):
   ```shell
   pip install -e .
   ```
...
## Making simultaneous changes to `romancal` and one of its dependencies

If you need to make a change in `romancal` that requires a simultaneous change to one of its dependencies (`stcal`, `roman-datamodels`, `stpipe`, etc.), also install that dependency from your local machine [to your development environment](#creating-a-development-environment).
For instance, assuming you've cloned the source code for both `romancal` and `stcal`, you can do the following from the `romancal/` directory:
```shell
cd romancal/
pip install -e .
pip install -e ../stcal
...
> ```shell
> mamba create -n romancal_stcal_dev_env python=3.13
> mamba activate romancal_stcal_dev_env
> ```

Since we do not use a single repository (sometimes called a "monorepo") for these coupled packages, when making a change like this, you will need to make two pull requests: one in the `romancal` repository, and one in the dependency's repository.
However, unit tests will not automatically pass because the `pyproject.toml` file in `romancal` points to the last released version of `stcal`, which does not incorporate your changes.
To resolve this (temporarily, for testing) modify the `pyproject.toml` files in `romancal` to point to your branch, to demonstrate that unit tests pass.
```toml
