...
There are a few things it needs to do:
   ...
   1) It should contain all of the core metadata from the core schema
      that ships with `romancal.datamodels`.  In JSON Schema parlance, this
...
``romancal.stsci.edu`` is recognized as a "special" address by the
...
.. note::
  ...
  different implementation of the `romancal.datamodels` framework possibly
  ...
  inherit from another, and the `romancal.datamodels` framework won't and
...
the docstring of `romancal.datamodels.DataModel.__init__` for more
...
.. code-block:: python
    ...
    def __init__(self, init=None, dq=None, dq_def=None, **kwargs):
        ...
        ----------
        init : any
            Any of the initializers supported by `~romancal.datamodels.DataModel`.
...
.. note::
   ...
   `romancal.datamodels` itself in the future.  However, this still stands
