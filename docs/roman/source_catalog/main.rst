...
:ref:`data_quality_flags`. Pixels with ``DO_NOT_USE`` set are masked
before sources are detected, so they never belong to a segment: the
column summarizes the flags of the pixels that were actually used, and
never itself reports ``DO_NOT_USE``. It is zero when none of those pixels
was flagged, and for Level 3 products, which carry no ``dq`` array.

The ``image_flags2`` column contains the corresponding bitwise ``or`` of
the ``dq2`` flags, described in :ref:`informational_data_quality_flags`,
over those same pixels. No ``dq2`` flags are defined yet, so this column
...
When sources are detected, the segmentation output also includes the
following keys, which are needed to make forced source catalogs:

* ``detection_image``: The detection image used to measure the source
  positions and shapes. This is the convolved, background-subtracted
  image, or the multiband detection image for multiband catalogs.

* ``detection_image_unit``: A string giving the unit of
  ``detection_image`` (``"nJy"``).
...
The segmentation file must contain the ``detection_image`` array. If
the file has no ``detection_image_unit`` key, which is the case for
files made by earlier versions of the pipeline, the detection image
from a source catalog step file is assumed to be in nJy and a warning
is logged. The detection image from a multiband catalog step file is
assumed to be in the units of the input images.
