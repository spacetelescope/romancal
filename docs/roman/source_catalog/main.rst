...
The segmentation file must contain the ``detection_image`` array when it
has sources. If the file has no ``detection_image_unit`` key, which is the
case for files made by earlier versions of the pipeline, the detection
image from a source catalog step file is assumed to be in nJy and a
warning is logged. The detection image from a multiband catalog step file
is assumed to be in the units of the input images. If the forcing
segmentation has no sources, forced photometry returns an empty catalog.
