...
---------------------------
    ...
    - :meth:`Step.open_model <romancal.stpipe.step.Step.open_model>`: Open
      a `DataModel`
    ...
    - :meth:`Step.make_input_path
      <romancal.stpipe.step.Step.make_input_path>`: Create a file name to
      be used as input
    - :meth:`Step.save_model <romancal.stpipe.step.Step.save_model>`: Save a `DataModel` immediately
    - :attr:`Step.make_output_path
      <romancal.stpipe.step.Step._make_output_path>`: Create a file name
      to be used as output

Design
======

The :class:`~romancal.stpipe.step.Step` architecture is designed such that
...
following structure::
  ...
  class MyStep(romancal.stpipe.step.Step):

      spec = ''  # Desired configuration parameters

      def process(self, input):

          with romancal.datamodels.open(input) as input_model:
...
.. doctest-skip::
  ...
  >>> import jwst
  >>> input = romancal.datamodels.open('input_data.fits')
...
the file path to a data file, or a Roman
:class:`~romancal.datamodels.DataModel` object. The method
:meth:`~romancal.stpipe.step.Step.open_model` handles either type of
...
:meth:`~romancal.stpipe.step.Step.open_model` handles `Step`-specific
...
:meth:`~romancal.stpipe.step.Step.make_input_path` can be used to specify
the input directory location.

Input and Associations
----------------------

Many of the Roman calibration steps and pipelines expect an
:ref:`Association <associations>` file as input. When opened with
:meth:`~romancal.stpipe.step.Step.open_model`, a
:class:`~romancal.datamodels.ModelContainer` is returned. `ModelContainer`
...
be seen in the `calwebb_spec3` default configuration file::
  ...
  name = "Spec3Pipeline"
  class = "romancal.pipeline.Spec3Pipeline"
...
of :meth:`Step.save_model <romancal.stpipe.step.Step.save_model>` is the recommended over
directly calling :meth:`DataModel.save <romancal.datamodels.DataModel.save>`.
...
See :meth:`romancal.stpipe.step.Step.save_model` for further information.
...
<romancal.stpipe.step.Step.make_output_path>`. By default, calling
